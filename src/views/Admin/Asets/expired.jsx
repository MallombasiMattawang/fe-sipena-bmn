...
export default function AsetExpired() {
    //title page
    document.title = "Daftar Aset/Barang BMN Habis Masa Manfaat";
    ...
    const fetchData = async (pageNumber = 1, keywords = "", category = "", condition = "", status = "", location = "") => {
        ...
        await Api.get(`/api/admin/asets-expired?search=${keywords}&category=${category}&condition=${condition}&status=${status}&location=${location}&page=${page}`, {
            //header
            headers: {
                //header Bearer + Token
